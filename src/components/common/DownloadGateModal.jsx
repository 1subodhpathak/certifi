...
export default function DownloadGateModal({
  ...
  onClose,
  clerkUser,
  resourceType = "certificate_pdf",
  resourceId = "default",
  resourceName = "Certificate / Score Report PDF",
  onSuccessDownload,
}) {
  ...
  const handlePayRupee = async () => {
    ...
    try {
      ...
      const userId = clerkUser.id || clerkUser.userId || clerkUser._id;
      const orderData = await createPassOrder(userId, resourceType, resourceId);
      ...
      const options = {
        ...
        name: "Certifi by CareerSense",
        description: `₹1 Download Pass - ${resourceName}`,
        order_id: orderData.orderId,
        prefill: {
          name: clerkUser.fullName || clerkUser.name || clerkUser.firstName || "",
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-emerald-900/40 bg-slate-900 p-6 text-white shadow-2xl">
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 shadow-xs">
        ...
        <div className="mt-4 text-center">
          ...
          <p className="mt-2 text-xs font-medium leading-relaxed text-slate-300">
            Free tier users get 10,000 one-time AI tokens. Downloading official verified certificates and PDF score reports requires a nominal fee of <strong className="text-emerald-400 font-bold">₹1</strong> per document.
          </p>
        </div>
        ...
        <div className="mt-6 grid gap-3">
          <button
            ...
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 px-5 py-3 text-sm font-bold text-slate-950 shadow-md transition hover:from-emerald-400 hover:to-teal-400 active:translate-y-0.5 disabled:opacity-60"
          >
          ...
          <div className="relative my-1 flex items-center justify-center">
            ...
            <span className="absolute bg-slate-900 px-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">
              OR
            </span>
          </div>
          ...
          <a
            ...
            onClick={onClose}
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/5 px-5 py-3 text-xs font-bold text-slate-200 transition hover:bg-white/10"
          >
