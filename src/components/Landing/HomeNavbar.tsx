          ...
          {NAV_SECTIONS.map((item) => {
            ...
            return (
              <a
                ...
                className={`relative rounded-md py-2 transition-colors hover:text-teal-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500/40 ${isActive ? 'text-teal-700' : ''
                  }`}
              >
                {item.label}
                <span
                  aria-hidden="true"
                  className={`absolute inset-x-1 -bottom-0.5 h-0.5 rounded-full bg-teal-500 transition-[transform,opacity] duration-200 motion-reduce:transition-none ${isActive ? 'scale-x-100 opacity-100' : 'scale-x-50 opacity-0'
                    }`}
                />
              </a>
            );
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          <SignedOut>
            <SignInButton mode="modal">
              <button
                type="button"
                className="rounded-lg px-4 py-2 text-sm font-bold text-white transition bg-teal-600 hover:bg-teal-700"
              >
                Log in
              </button>
            </SignInButton>
            ...
            <div className="grid gap-2 text-[13px] font-semibold text-slate-700">
              {NAV_SECTIONS.map((item) => (
                <a
                  ...
                  className={`rounded-2xl border px-4 py-3 shadow-sm transition ${activeSection === item.id
